...
FLOWER_ICON = (
    "iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAB6ElEQVR4nO2V"
    "z0sCURDHd1X8LSZroHQJM1bIv8Crp/Bv8BQeNsJzf4DQpboK/gHhoVMUkSh0"
    "sOg/ELJbB7vlpYsQ23f0G2xGme1qBDvwgee8eTOz8+aNiuKKK/9ZTON8W/ir"
    "4FlwTLJLDX73MFQRtAJuSUV0ywjsAX4QQtAGeCYN0XHPs6jgPgaJgRUE7QKT"
    "dEXHPbHxLSJ4GMRBEgHz4NGSgKzzskebsCNJwKmUOsCvEserYA26PTDCelOQ"
    "tehkjzZxXlNAfPwmcA7sgjo4Ax1wBZq8+9b4y3vKhEkVWtxr0rbDs3X6ys2T"
    "QBrsgzZ4spTa/BB4mmm7ydk2faXnrUIGHIHeu0OUdQNsfZWA7ImNJYEefWTm"
    "vYWxSCPhsAFuZn795yrIGcNWM7L7o3CUAAeg/4ME+rRNyFm7CXhBBGjs/hoY"
    "fhN8KDZ8DRrPeu0koPIJypBJsctnlV+mYopnQrZHNMdrDI51cM9m1K3NyObT"
    "mYDY6JyKflvBmYCHQ6XGgWNttAti1Y14DSHH/hfgMAguwSuDvPA61kmDOpM2"
    "Yht0JDgTKIFrMGCXH7LJkkQTHfcGtC05mUAZnIBTUGV5I3xmUa7lmqq0Eduy"
    "kwkUwQ4oyG++Di/nhI9rlbYF2hYdS8AVV1xZpLwBacYSsjhGyZUAAAAASUVO"
    "RK5CYII="
)
...
class App(tk.Tk):
    ...
    def _install_update(self, temp_path, dialog):
        ...
        if system == "Windows":
            update_ps1 = os.path.join(os.path.dirname(temp_path), "update.ps1")
            with open(update_ps1, "w") as f:
                f.write('$temp = "' + temp_path + '"\n')
                f.write('$target = "' + app_path + '"\n')
                f.write('Write-Host "Waiting for the app to close..."\n')
                ...
                f.write('  $retry++\n')
                f.write('  try {\n')
                f.write('    Copy-Item $temp $target -Force -ErrorAction Stop\n')
                f.write('    $ok = $true\n')
                f.write('  } catch {\n')
                f.write('    if ($retry -lt 5) {\n')
                f.write('      Write-Host "Retry $retry..."\n')
                f.write('      Start-Sleep -Seconds 2\n')
                f.write('    }\n')
                f.write('  }\n')
                f.write('} while (-not $ok -and $retry -lt 5)\n')
                f.write('if ($ok) {\n')
                f.write('  Start-Process $target\n')
                f.write('  Remove-Item $PSCommandPath -Force\n')
                f.write('} else {\n')
                f.write('  Write-Host "`nUpdate failed."\n')
                f.write('  Write-Host "Temp: $temp"\n')
                f.write('  Write-Host "Target: $target"\n')
                f.write('  Write-Host "Copy manually and restart.`n"\n')
                f.write('  Add-Type -AssemblyName System.Windows.Forms\n')
                f.write('  $null = [System.Windows.Forms.MessageBox]::Show(\n')
                f.write('    "Update failed.`nTemp: $temp`nTarget: $target`n`nCopy the temp file manually.",\n')
                f.write('    "Table Generator Update",\n')
                f.write('    "OK",\n')
                f.write('    "Warning")\n')
                f.write('  Read-Host "Press Enter to exit"\n')
                f.write('}\n')
            subprocess.Popen(["powershell", "-WindowStyle", "Normal",
