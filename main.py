    ...
    def _install_update(self, temp_path, dialog):
        ...
        if system == "Windows":
            ...
            with open(update_bat, "w") as f:
                ...
                f.write('title Table Generator Updater\n')
                f.write('echo Waiting for the app to close...\n')
                ...
                f.write('  if !RETRIES! lss 5 (\n')
                f.write('    echo Retry !RETRIES!...\n')
                f.write('    timeout /t 2 /nobreak >nul\n')
                f.write('    goto retry\n')
                f.write('  )\n')
                f.write('  echo.\n')
                f.write('  echo =ERROR====================================\n')
                f.write('  echo Could not replace the executable.\n')
                f.write('  echo.\n')
                f.write('  echo Temp file: "%USERPROFILE%\\.table_generator_update\\TableGenerator.exe"\n')
                f.write('  echo Target:     "' + app_path + '"\n')
                f.write('  echo.\n')
                f.write('  echo Copy the temp file manually to the target path.\n')
                f.write('  echo ==========================================\n')
                f.write('  echo.\n')
                f.write('  msg * "Update failed. The file could not be replaced. Copy the temp file manually."\n')
                f.write('  pause\n')
                f.write('  exit /b 1\n')
                f.write(')\n')
